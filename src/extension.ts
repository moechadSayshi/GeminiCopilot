...
export function activate(context: vscode.ExtensionContext) {
  ...
  async function updateStatusBar() {
    ...
    if (now < cooldownUntil) {
      ...
      statusBarItem.tooltip =
        "Gemini Copilot is active. Click to change Gemini API Key.";
  ...
  const setApiKeyCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      vscode.window.showInformationMessage(
        "Gemini API Key has been saved securely.",
      );
    },
  );
  ...
  const provider: vscode.InlineCompletionItemProvider = {
    ...
    > {
      ...
      if (!apiKey) {
        log(
          "Completions skipped: Gemini API Key is not configured. Run 'Gemini Copilot: Set Gemini API Key'.",
        );
        return undefined;
      }
      ...
      log(
        `Requesting completion for ${path.basename(document.fileName)} (${document.languageId})`,
      );
      ...
      const cancelSubscription = token.onCancellationRequested(() => {
        log(
          "Keystroke cancellation triggered. Aborting active Gemini API request.",
        );
      ...
      try {
        ...
        if (error.name === "AbortError") {
          ...
          if (error.message && error.message.includes("HTTP 429")) {
            let delaySeconds = 60;
            try {
              const errorJsonStr = error.message.substring(
            ...
            log(
              `Rate limit reached. Entering cooldown for ${delaySeconds} seconds.`,
            );
